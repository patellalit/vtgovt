      ...
      <div class="page-header users-header">
        <h2>
		  ...
		  <?php
			...
			{
		  ?>
          		<a  href="<?php echo site_url("").$this->uri->segment(1); ?>/add" class="btn btn-success">નવી પ્રવૃત્તિ ઉમેરો</a>
			<?php
			}
			?>
        </h2>
        ...
        <div class="span12 columns">
          <div class="well">
            ...
            $attributes = array('class' => 'form-inline reset-margin', 'id' => 'myform','method'=>'GET');
			 $options_agegroup = array(0 => "all");
            foreach ($agegroup as $row)
            {
              $options_agegroup[$row['id']] = $row['name'];
            }
            echo form_open('activities', $attributes);
			
			echo form_label('વય જૂથ', 'agegroup_id');
              echo form_dropdown('agegroup_id', $options_agegroup, $agegroup_selected, 'class="span2"');
          ...
          <table class="table table-striped table-bordered table-condensed">
            <thead>
              <tr>
                ...
                <th class="yellow header headerSortDown">પ્રવૃત્તિઓ નામ</th>
            ...
            <tbody>
              ...
              foreach($activities as $row)
              {
                ...
                echo '<a href="'.site_url("").'activities/update/'.$row['id'].'" class="btn btn-info">View & Edit</a>'; 
				if($this->session->userdata('is_admin')==true)
				{ 
                	echo '<a href="javascript:void(0)" onclick="if(confirm(\'Are you sure you want to delete this activity?\')){document.location.href=\''.site_url("").'activities/delete/'.$row['id'].'\'}" class="btn btn-danger">Delete</a>';
				}
                echo '</td>';
                echo '</tr>';
              }
	...
	<script type="text/javascript">
		...
		function changePage(e,textbox)
		{
			...
			if(code == 13 && document.getElementById('gotopage').value!='') { //Enter keycode
			 	//document.location.href='<?php echo base_url('activities/page') ?>/'+document.getElementById('gotopage').value+'/'+document.getElementById('rowsperpage').value;
				$('#perpage').val($('#rowsperpage').val());
				$('#currentpage').val($('#gotopage').val());
				$("#myform").attr("action", '<?php echo base_url('activities/page') ?>/'+document.getElementById('gotopage').value);
				$('#myform').submit();
			}
			else
			{
		...
		{
			//document.location.href='<?php echo base_url('activities/page/'.$currentpage) ?>/'+document.getElementById('rowsperpage').value;			
			$('#perpage').val($('#rowsperpage').val());
					$('#currentpage').val($('#gotopage').val());
					$("#myform").attr("action", '<?php echo base_url('activities/page') ?>/'+document.getElementById('gotopage').value);
					$('#myform').submit();
