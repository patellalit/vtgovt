      ...
      <div class="page-header users-header">
        <h2>
		  ...
		  <?php
			...
			{
		  ?>
          		<a  href="<?php echo site_url("").$this->uri->segment(1); ?>/add" class="btn btn-success">નવી વય જૂથ ઉમેરો</a>
			<?php
			}
			?>
        </h2>
      ...
      <div class="row">
        <div class="span12 columns">
          <div class="well">
            ...
            echo form_open('agegroup', $attributes);
          ...
          <table class="table table-striped table-bordered table-condensed">
            <thead>
              <tr>
                ...
                <th class="yellow header headerSortDown">વય જૂથ નામ</th>
            ...
            <tbody>
              ...
              foreach($agegroup as $row)
              {
                ...
                echo '<a href="'.site_url("").'agegroup/update/'.$row['id'].'" class="btn btn-info">View & Edit</a>'; 
				if($this->session->userdata('is_admin')==true)
				{ 
                	echo '<a href="javascript:void(0)" onclick="if(confirm(\'Are you sure you want to delete this agegroup?\')){document.location.href=\''.site_url("").'agegroup/delete/'.$row['id'].'\'}" class="btn btn-danger">Delete</a>';
				}
                echo '</td>';
                echo '</tr>';
              }
	...
	<script type="text/javascript">
		...
		function changePage(e,textbox)
		{
			...
			if(code == 13 && document.getElementById('gotopage').value!='') { //Enter keycode
			 	//document.location.href='<?php echo base_url('agegroup/page') ?>/'+document.getElementById('gotopage').value+'/'+document.getElementById('rowsperpage').value;
				$('#perpage').val($('#rowsperpage').val());
				$('#currentpage').val($('#gotopage').val());
				$("#myform").attr("action", '<?php echo base_url('agegroup/page') ?>/'+document.getElementById('gotopage').value);
				$('#myform').submit();
			}
			else
			{
		...
		{
			//document.location.href='<?php echo base_url('agegroup/page/'.$currentpage) ?>/'+document.getElementById('rowsperpage').value;			
			$('#perpage').val($('#rowsperpage').val());
					$('#currentpage').val($('#gotopage').val());
					$("#myform").attr("action", '<?php echo base_url('agegroup/page') ?>/'+document.getElementById('gotopage').value);
					$('#myform').submit();
