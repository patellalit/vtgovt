      ...
      <div class="page-header users-header">
        <h2>
		  ...
		  <?php
			...
			{
		  ?>
          		<a  href="<?php echo site_url("").$this->uri->segment(1); ?>/add" class="btn btn-success">નવી વસ્તુ ઉમેરો</a>
			<?php
			}
			?>
        </h2>
          ...
          <table class="table table-striped table-bordered table-condensed">
            ...
            <tbody>
              ...
              foreach($item as $row)
              {
                ...
                echo '<a href="'.site_url("").'item/update/'.$row['id'].'" class="btn btn-info">View & Edit</a>'; 
				if($this->session->userdata('is_admin')==true)
				{ 
                	echo '<a href="javascript:void(0)" onclick="if(confirm(\'Are you sure you want to delete this item?\')){document.location.href=\''.site_url("").'item/delete/'.$row['id'].'\'}" class="btn btn-danger">Delete</a>';
				}
                echo '</td>';
                echo '</tr>';
              }
