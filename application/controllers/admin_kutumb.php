...
class Admin_kutumb extends CI_Controller {
    ...
    */
    public function index()
    {
        ...
        $jati_id = $this->input->get('jati_id');
        $religion_id = $this->input->get('religion_id');
        $laghumati = $this->input->get('laghumati');
        if($perpagePost != '')
		{
			$perPage = $perpagePost;
		}
		...
		$config['base_url'] = base_url().'kutumb?'.http_build_query($_GET);
        ...
        if($laghumati != '' || $laghumati != 0)
			...
			$data['count_kutumb'] = $this->kutumb_model->get_kutumb('','','',$aanganwadiid_id, $searchtxt,$jati_id,$religion_id,$laghumati, '', null, null,null,'total');
			$data['count_kutumb_khodkhapan'] = $this->kutumb_model->get_kutumb('','','',$aanganwadiid_id, $searchtxt,$jati_id,$religion_id,$laghumati, '', null, null,null,'khodkhapan');
			$data['count_kutumb_death'] = $this->kutumb_model->get_kutumb('','','',$aanganwadiid_id, $searchtxt,$jati_id,$religion_id,$laghumati, '', null, null,null,'death');
			$data['count_kutumb_birth'] = $this->kutumb_model->get_kutumb('','','',$aanganwadiid_id, $searchtxt,$jati_id,$religion_id,$laghumati, '', null, null,null,'birth');
	...
	public function addkutumb()
    {
        ...
        {	
            ...
            $this->form_validation->set_error_delimiters('<div class="alert alert-error"><a class="close" data-dismiss="alert">\D7</a><strong>', '</strong></div>');
            ...
            if ($this->form_validation->run())
            {
				...
				if($kutumbid!=0)
				{
					for($i=0;$i<count((array)$_REQUEST['familypersondata']);$i++)
					{
						...
						if($_REQUEST['familypersondata']->$i->txtBirthDate == ""){
							$_REQUEST['familypersondata']->$i->txtBirthDate = null;
						}
						if($_REQUEST['familypersondata']->$i->txtOutSthadantrarDate == ""){
							$_REQUEST['familypersondata']->$i->txtOutSthadantrarDate = null;
						}
						if($_REQUEST['familypersondata']->$i->txtDeathDate== ""){
							$_REQUEST['familypersondata']->$i->txtDeathDate = null;
						}
						if($_REQUEST['familypersondata']->$i->txtsthantarDate== ""){
							$_REQUEST['familypersondata']->$i->txtsthantarDate = null;
						}
						if($_REQUEST['familypersondata']->$i->lmp_date== ""){
							$_REQUEST['familypersondata']->$i->lmp_date = null;
						}
						
						if($_REQUEST['familypersondata']->$i->miscarage_date== ""){
							$_REQUEST['familypersondata']->$i->miscarage_date = null;
						}
						if($_REQUEST['familypersondata']->$i->nondhani_date== ""){
							$_REQUEST['familypersondata']->$i->nondhani_date = null;
						}
                        $childcategory=0;
                        
                        $birthday = new DateTime($_REQUEST['familypersondata']->$i->txtBirthDate);
                        $diff = $birthday->diff(new DateTime());
                        $month = $diff->format('%m') + 12 * $diff->format('%y');
                        
                        if($month <= 72)
                        {
                            if($_REQUEST['familypersondata']->$i->drpKhodkhapan == 1 || $_REQUEST['familypersondata']->$i->drpKhodkhapan == 2)
                            {
                                $childcategory = '1';
                            }
                            else
                        ...
                        if($_REQUEST['familypersondata']->$i->drplakshyank == 6)
                            $childcategory = '6';
                        if($_REQUEST['familypersondata']->$i->drplakshyank == 4)
                            $childcategory = '5';
           ...
           //form validation
            ...
            if ($this->form_validation->run())
            {
                ...
                if($this->kutumb_model->update_kutumb($id, $data_to_store) == TRUE){
					...
					for($i=0;$i<count((array)$_REQUEST['familypersondata']);$i++)
					{
                        /*if($_REQUEST['familypersondata']->$i->family_person_id==1129)
                        {print_r($_REQUEST['familypersondata']->$i);exit;}*/
						$_REQUEST['familypersondata']->$i->txtDeathDate = str_replace("/","-",$_REQUEST['familypersondata']->$i->txtDeathDate);
						$_REQUEST['familypersondata']->$i->txtOutSthadantrarDate = str_replace("/","-",$_REQUEST['familypersondata']->$i->txtOutSthadantrarDate);
						$_REQUEST['familypersondata']->$i->txtsthantarDate = str_replace("/","-",$_REQUEST['familypersondata']->$i->txtsthantarDate);
						$_REQUEST['familypersondata']->$i->txtBirthDate = str_replace("/","-",$_REQUEST['familypersondata']->$i->txtBirthDate);
						...
						if($_REQUEST['familypersondata']->$i->txtDeathDate != '')
                            ...
                            $_REQUEST['familypersondata']->$i->txtDeathDate = '0000-00-00';
						
						if($_REQUEST['familypersondata']->$i->txtOutSthadantrarDate != '')
							$_REQUEST['familypersondata']->$i->txtOutSthadantrarDate = date('Y-m-d',strtotime($_REQUEST['familypersondata']->$i->txtOutSthadantrarDate));
                        else
                            $_REQUEST['familypersondata']->$i->txtOutSthadantrarDate = '0000-00-00';
						
						if($_REQUEST['familypersondata']->$i->txtsthantarDate != '')
							$_REQUEST['familypersondata']->$i->txtsthantarDate = date('Y-m-d',strtotime($_REQUEST['familypersondata']->$i->txtsthantarDate));
                        else
                            $_REQUEST['familypersondata']->$i->txtsthantarDate = '0000-00-00';
						
						if($_REQUEST['familypersondata']->$i->txtBirthDate != '')
							$_REQUEST['familypersondata']->$i->txtBirthDate = date('Y-m-d',strtotime($_REQUEST['familypersondata']->$i->txtBirthDate));
                        else
                            $_REQUEST['familypersondata']->$i->txtBirthDate = '0000-00-00';
							
						if($_REQUEST['familypersondata']->$i->lmp_date != '')
							$_REQUEST['familypersondata']->$i->lmp_date = date('Y-m-d',strtotime($_REQUEST['familypersondata']->$i->lmp_date));
						else
							$_REQUEST['familypersondata']->$i->lmp_date='0000-00-00';
			
		
						if($_REQUEST['familypersondata']->$i->miscarage_date != '')
							$_REQUEST['familypersondata']->$i->miscarage_date = date('Y-m-d',strtotime($_REQUEST['familypersondata']->$i->miscarage_date));
						else
							$_REQUEST['familypersondata']->$i->miscarage_date='0000-00-00';
			
						if($_REQUEST['familypersondata']->$i->nondhani_date != '')
							$_REQUEST['familypersondata']->$i->nondhani_date = date('Y-m-d',strtotime($_REQUEST['familypersondata']->$i->nondhani_date));
						else
							$_REQUEST['familypersondata']->$i->nondhani_date='0000-00-00';
						...
						//echo $_REQUEST['familypersondata']->$i->family_person_id;
							...
							if($lastLMPdate != $_REQUEST['familypersondata']->$i->lmp_date && $_REQUEST['familypersondata']->$i->lmp_date != '' && $_REQUEST['familypersondata']->$i->lmp_date != '0000-00-00')
							{
