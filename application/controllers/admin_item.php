...
class Admin_item extends CI_Controller {
    ...
    */
    public function index()
    {
        ...
        $config['base_url'] = base_url().'item/page?'.http_build_query($_GET);
		...
		$data['count_item']= $this->item_model->count_item($search);
		$data['item'] = $this->item_model->get_item($search, '', $order_type, $config['per_page'],$limit_end);        
	...
	public function add()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
            ...
            {
				$isitem = $this->item_model->if_item_exists($this->input->post('item_name'),0);
				if(empty($isitem))
				{
					$data_to_store = array(
						'item_name' => $this->input->post('item_name'),
					);
					$item_id=$this->item_model->store_item($data_to_store);
